...
var (
	store = flag.String("store", "memory", "registered store name, [memory, goleveldb, boltdb, tikv]")
	path  = flag.String("path", "test/%s", "store path")
)
...
func (m sqliteDB) RunTest(t *testing.T, fn func(params)) {
...
func (o *tiDB) RunTest(t *testing.T, fn func(params)) {
	if !o.Running() {
		t.Logf("skipping test; no TiDB")
		return
	}
	flag.Parse()
	tidb.SetSchemaLease(0)
	dbName := "test"
	db, err := sql.Open("mysql", "root@tcp(localhost:4001)/"+dbName+"?strict=true")
	if err != nil {
		t.Fatalf("error connecting: %v", err)
	}

	params := params{ti, t, db}
...
func TestBlobs_TiDB(t *testing.T) { ti.RunTest(t, testBlobs) }
...
func TestManyQueryRow_TiDB(t *testing.T) { ti.RunTest(t, testManyQueryRow) }
...
func TestTxQuery_TiDB(t *testing.T) { ti.RunTest(t, testTxQuery) }
...
func TestPreparedStmt_TiDB(t *testing.T) { ti.RunTest(t, testPreparedStmt) }
