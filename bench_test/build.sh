...
make
cp -f ./bin/tidb-server $TIDB_PATH/bin/bench-tidb-server
cd $TIDB_PATH
echo "build TiDB OK"
...
make
cp -f ./bin/pd-server $TIDB_PATH/bin/bench-pd-server
...
cp -f ./bin/tikv-server $TIDB_PATH/bin/bench-tikv-server
cd $TIDB_PATH
echo "build TiKV OK"
