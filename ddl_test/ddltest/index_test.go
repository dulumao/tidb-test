...
func (s *TestDDLSuite) checkAddIndex(c *C, indexInfo *model.IndexInfo) {
	...
	idx := tables.NewIndex(tbl.Meta(), indexInfo)
	...
	for i := 0; i < workerNum; i++ {
		go func() {
			...
			for j := 0; j < count; j++ {
				...
				s.execInsert(c, fmt.Sprintf("insert into test_index values (%d, %d, %f, '%s')",
					id, randomInt(), randomFloat(), randomString(10)))
				s.mustExec(c, fmt.Sprintf("delete from test_index where c = %d", randomIntn(int(id))))
				s.mustExec(c, fmt.Sprintf("update test_index set c1 = %d, c2 = %f, c3 = '%s' where c = %d",
					randomInt(), randomFloat(), randomString(10), randomIntn(int(id))))
			}
		}()
	}
	wg.Wait()
