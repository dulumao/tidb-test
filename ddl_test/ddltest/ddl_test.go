...
func (s *TestDDLSuite) SetUpSuite(c *C) {
	...
	var err error
	s.store, err = tidb.NewStore(fmt.Sprintf("tikv://%s%s", *etcd, *tikvPath))
	c.Assert(err, IsNil)
	s.s, err = tidb.CreateSession(s.store)
	c.Assert(err, IsNil)

	s.ctx = s.s.(context.Context)

	// Set a little schema lease to speed up initialization.
	sessionctx.GetDomain(s.ctx).SetLease(100 * time.Millisecond)

	_, err = s.s.Execute("create database if not exists test_ddl")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("use test_ddl")
	c.Assert(err, IsNil)

	s.Bootstrap(c)

	// Stop current DDL worker, so that we can't be the owner now.
	err = sessionctx.GetDomain(s.ctx).DDL().Stop()
	c.Assert(err, IsNil)
	...
	for {
		...
		case <-time.After(time.Duration(after) * time.Second):
			if *enableRestart {
				...
				if err != nil {
					log.Fatalf("restartServerRand failed, err %v", err)
				}
			}
		case <-s.quit:
			err = s.stopServers()
			if err != nil {
				log.Fatalf("stopServers failed, err %v", err)
			}
			return
		}
	}
...
func (s *TestDDLSuite) killServer(proc *os.Process) error {
	...
	c := fmt.Sprintf("kill -9 %d", proc.Pid)
...
func (s *TestDDLSuite) getTable(c *C, name string) table.Table {
	tbl, err := sessionctx.GetDomain(s.ctx).InfoSchema().TableByName(model.NewCIStr("test_ddl"), model.NewCIStr(name))
	c.Assert(err, IsNil)
	return tbl
}
...
func (s *TestDDLSuite) Bootstrap(c *C) {
	...
	_, err = s.s.Execute("drop table if exists test_index, test_column, test_insert, test_conflict_insert, " +
		"test_update, test_conflict_update, test_delete, test_conflict_delete, test_mixed, test_inc")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_index (c int, c1 bigint, c2 double, c3 varchar(256), primary key(c))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_column (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_insert (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_conflict_insert (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_update (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_conflict_update (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_delete (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_conflict_delete (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_mixed (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)

	_, err = s.s.Execute("create table test_inc (c1 int, c2 int, primary key(c1))")
	c.Assert(err, IsNil)
}
